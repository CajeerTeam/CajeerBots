...
## 0.10.1 dry-run and migration safety

Before applying a release, use:

```bash
cajeer-bots update plan --version latest
cajeer-bots update apply --version latest --dry-run
cajeer-bots db backup --format custom
```

Stable releases should be installed from signed GitHub Release artifacts. Source ZIP snapshots are not production artifacts unless generated by `scripts/build_release_zip.py`.
