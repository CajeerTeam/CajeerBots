...
class BridgeModule:
    ...
    async def on_start(self, context) -> None:
        context.logger.info("модуль bridge запущен")
    ...
    async def on_command(self, command: str, event: CajeerEvent, context) -> dict[str, object] | None:
        if command != "bridge":
            return None
        return {"ok": True, "message": "Событие передано через bridge module.", "trace_id": event.trace_id}

    async def on_stop(self, context) -> None:
        context.logger.info("модуль bridge остановлен")
