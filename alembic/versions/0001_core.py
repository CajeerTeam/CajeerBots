...
SCHEMA = validate_schema_name(os.getenv("DATABASE_SCHEMA_SHARED", "shared"))
...
def upgrade() -> None:
    ...
    op.execute(
        f"INSERT INTO {SCHEMA}.platform_schema(component, version) "
    ...
    op.create_table("roles", sa.Column("role_id", sa.String(64), primary_key=True), sa.Column("title", sa.String(255), nullable=False), sa.Column("source", sa.String(64), nullable=False, server_default="local"), sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")), schema=SCHEMA)
    op.create_table("role_permissions", sa.Column("role_id", sa.String(64), primary_key=True), sa.Column("permission", sa.String(128), primary_key=True), schema=SCHEMA)
    op.create_table("user_roles", sa.Column("user_id", sa.String(64), primary_key=True), sa.Column("role_id", sa.String(64), primary_key=True), sa.Column("granted_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")), schema=SCHEMA)
    op.create_table("support_tickets", sa.Column("ticket_id", sa.String(64), primary_key=True), sa.Column("user_id", sa.String(64), nullable=True, index=True), sa.Column("platform", sa.String(64), nullable=False), sa.Column("platform_chat_id", sa.String(128), nullable=False, index=True), sa.Column("status", sa.String(32), nullable=False, server_default="open", index=True), sa.Column("subject", sa.String(255), nullable=False, server_default=""), sa.Column("assigned_to", sa.String(128), nullable=True, index=True), sa.Column("history", postgresql.JSONB(astext_type=sa.Text()), nullable=False), sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")), sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")), schema=SCHEMA)
    ...
    op.create_table("user_profiles", sa.Column("user_id", sa.String(64), primary_key=True), sa.Column("profile", postgresql.JSONB(astext_type=sa.Text()), nullable=False), sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")), schema=SCHEMA)
