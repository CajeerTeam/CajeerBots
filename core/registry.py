...
class Registry:
    ...
    def _load(self, path: Path) -> Manifest:
        ...
        return Manifest(
            id=data["id"], name=data.get("name", data["id"]), version=data.get("version", "0.0.0"),
            type=data.get("type", "module"), path=path.parent, description=data.get("description", ""),
            requires=tuple(data.get("requires", [])), adapters=tuple(data.get("adapters", [])),
            capabilities=tuple(data.get("capabilities", [])), enabled_by_default=bool(data.get("enabled_by_default", False)),
        )
    ...
    def validate(self) -> list[str]:
        ...
        for manifest in self.all():
            ...
            if key in seen:
                errors.append(f"duplicate manifest id: {key}")
            seen.add(key)
            if not manifest.version:
                errors.append(f"manifest {key} has empty version")
        return errors
